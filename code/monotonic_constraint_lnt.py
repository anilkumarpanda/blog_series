...
non_mono_model = show_model_results(
    data=data_dict, model=non_mono_lgb, calc_threshold=False
)
...
model_params["monotone_constraints"] = get_monotone_constraints(
    data_dict=data_dict, target=target
)
logger.info(f"Creating model with params : {model_params}")
